...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
        ...
        $this->set_message('Please check your email address for activation');
	...
	public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, first_name, user_pic')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
        ...
        $log = array(
            ...
            'activity' => 'Login',
            'ip_address' => $this->input->ip_address(),
            'desc' => 'Login Failed using password |' . $password_nohas
        );
        $this->auth_log($log);
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, last_login, user_pic')
